...
1. 在 Kick 开发者门户创建应用。
2. 注册重定向 URI
3. 在 EsperantAI 中打开：**平台事件**面板 → **Kick**
4. 粘贴 Client ID 并点击 **连接**
5. Kick 使用带 PKCE 的 OAuth 2.1。

当前状态：**测试版/有限支持**。Kick 官方文档使用 webhook 接收完整事件。在浏览器内，EsperantAI 只能检测一部分有限活动；如果需要可靠的 Kick 订阅、礼物、raid 或完整事件，请使用 StreamElements 这类桥接，或使用后端/webhook。
...
EsperantAI 代码中包含 Trovo 技术适配器，基于 OAuth 和 Trovo WebSocket 聊天服务。当前公共界面还没有 Trovo 连接面板，因此它不作为普通用户流程记录。如果你现在需要 Trovo，请使用兼容桥接，或等待 Trovo 面板启用。
...
Kick 在浏览器中处于测试版/有限支持。Kick 的完整事件官方通过 webhook 接收；如果需要可靠的订阅、礼物或 raid，请使用 StreamElements 或你自己的后端。
