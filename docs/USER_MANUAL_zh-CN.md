...
2. 注册重定向 URI：`https://TU-DOMINIO/oauth-callback.html`（或你的本地 URL）
...
### 通过 Streamer.bot 连接 Kick

EsperantAI 通过 **Streamer.bot 桥接**接收 Kick 事件。这是推荐的可销售方案，因为它不会在浏览器中暴露 Kick 密钥，也不依赖逆向工程。

1. 安装 Streamer.bot 1.0.0 或更高版本。
2. 在 Streamer.bot 中连接你的 Kick 账号。
3. 在 Streamer.bot 中打开 **Servers/Clients -> WebSocket Server** 并启用服务器。
4. 使用 `127.0.0.1`、端口 `8080` 和 endpoint `/`，除非你修改过这些值。
5. 在 EsperantAI：**平台事件** 面板 -> **Kick via Streamer.bot**。
6. 点击 **连接**。

可用事件取决于 Streamer.bot 中启用的 Kick 集成。Kick 官方 backend/webhook 集成仍属于高级路线图。
...
EsperantAI 通过 OAuth 和 Trovo 官方聊天 WebSocket 连接 Trovo。

1. 在 Trovo 开发者门户创建应用。
2. 在打开 EsperantAI 的同一域名下注册重定向 URI：`https://TU-DOMINIO/oauth-callback.html`。
3. 在 EsperantAI：**平台事件** 面板 -> **Trovo**。
4. 粘贴你的 Client ID 并点击 **连接**。
5. 授权所需权限。

可用事件取决于 Trovo 聊天消息和官方聊天 token 流程。
...
### Kick 无法通过 Streamer.bot 连接

确认 Streamer.bot 1.0.0+ 已打开，Kick 已在 Streamer.bot 内连接，并且 **WebSocket Server** 已启用。除非修改过配置，否则使用 `127.0.0.1:8080/`。如果 Streamer.bot 要求密码，请在 EsperantAI 中输入相同密码。
...
- 📚 网页手册：https://edugame.digital/docs/manual.html
