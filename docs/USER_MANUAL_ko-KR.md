...
- **Kick**: 브라우저 기반 베타/제한 지원. Kick의 전체 이벤트를 쓰려면 공식 백엔드/웹훅 또는 브리지가 필요합니다.
- **StreamElements**: 계정의 토큰/JWT를 사용하는 멀티플랫폼 브리지.
- **Trovo**: 코드에는 기술 어댑터가 있지만, 현재 공개 UI에는 연결 패널이 아직 노출되어 있지 않습니다.
...
1. Kick 개발자 포털에서 애플리케이션을 만듭니다.
2. 리디렉트 URI를 등록합니다.
3. EsperantAI에서 **플랫폼 이벤트** 패널 → **Kick**을 엽니다.
4. Client ID를 붙여넣고 **연결**을 클릭합니다.
5. Kick은 PKCE가 포함된 OAuth 2.1을 사용합니다.

현재 상태: **베타/제한**. Kick 공식 문서는 전체 이벤트에 웹훅을 사용합니다. 브라우저에서 EsperantAI가 감지할 수 있는 활동은 일부로 제한됩니다. Kick 구독, 선물, 레이드 또는 신뢰도 높은 이벤트가 필요하다면 StreamElements 같은 브리지나 백엔드/웹훅을 사용하세요.
...
EsperantAI 코드에는 Trovo의 OAuth와 WebSocket 채팅 서비스를 기반으로 한 기술 어댑터가 포함되어 있습니다. 하지만 현재 공개 인터페이스에는 Trovo 연결 패널이 아직 없으므로 일반 사용자 흐름으로 문서화하지 않습니다. 지금 Trovo가 필요하다면 호환되는 브리지를 사용하거나 Trovo 패널이 활성화될 때까지 기다리세요.
...
### Kick이 모든 이벤트를 표시하지 않음

Kick은 브라우저에서 **베타/제한** 모드입니다. Kick의 전체 이벤트는 공식적으로 웹훅으로 수신됩니다. 신뢰도 높은 구독, 선물, 레이드 이벤트가 필요하다면 StreamElements 또는 자체 백엔드를 사용하세요.
