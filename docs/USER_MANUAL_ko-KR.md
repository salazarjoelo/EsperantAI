...
- **Kick**: 로컬 **Streamer.bot 브리지**로 지원합니다. EsperantAI는 브라우저에 Kick 비밀값을 저장하지 않습니다.
- **StreamElements**: 계정의 토큰/JWT를 사용하는 멀티플랫폼 브리지.
- **Trovo**: Trovo OAuth 및 공식 채팅 WebSocket을 통한 직접 지원.
...
2. 리디렉트 URI를 등록합니다: `https://TU-DOMINIO/oauth-callback.html`(또는 로컬 URL)
...
### Streamer.bot를 통한 Kick

EsperantAI는 **Streamer.bot 브리지**를 통해 Kick 이벤트를 받습니다. 브라우저에 Kick 비밀값을 노출하지 않고 리버스 엔지니어링에 의존하지 않기 때문에 판매용으로 권장되는 방식입니다.

1. Streamer.bot 1.0.0 이상을 설치합니다.
2. Streamer.bot에서 Kick 계정을 연결합니다.
3. Streamer.bot의 **Servers/Clients -> WebSocket Server**를 활성화합니다.
4. 변경하지 않았다면 `127.0.0.1`, 포트 `8080`, endpoint `/`를 사용합니다.
5. EsperantAI: **플랫폼 이벤트** 패널 -> **Kick via Streamer.bot**.
6. **연결**을 클릭합니다.

사용 가능한 이벤트는 Streamer.bot에서 활성화된 Kick 통합에 따라 달라집니다. Kick 공식 backend/webhook 통합은 고급 로드맵 항목입니다.
...
EsperantAI는 OAuth와 Trovo 공식 채팅 WebSocket을 통해 Trovo와 연결됩니다.

1. Trovo 개발자 포털에서 앱을 만듭니다.
2. EsperantAI를 여는 동일한 도메인에 리디렉트 URI `https://TU-DOMINIO/oauth-callback.html`를 등록합니다.
3. EsperantAI: **플랫폼 이벤트** 패널 -> **Trovo**.
4. Client ID를 붙여 넣고 **연결**을 클릭합니다.
5. 요청된 권한을 승인합니다.

사용 가능한 이벤트는 Trovo 채팅 메시지와 공식 채팅 토큰 플로우에 따라 달라집니다.
...
### Kick이 Streamer.bot로 연결되지 않음

Streamer.bot 1.0.0+이 열려 있고, Streamer.bot 안에서 Kick이 연결되어 있으며, **WebSocket Server**가 활성화되어 있는지 확인하세요. 설정을 변경하지 않았다면 `127.0.0.1:8080/`를 사용합니다. Streamer.bot가 비밀번호를 요구하면 EsperantAI에도 같은 비밀번호를 입력하세요.
...
- 📚 웹 매뉴얼: https://edugame.digital/docs/manual.html
